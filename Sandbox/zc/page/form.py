...
import zc.page.page
from zc.page import interfaces, namedtemplate
from zc.page.i18n import _
...
def render_submit_button(self):
    ...
    return ('<input type="submit" id="%s" name="%s" value="%s"'
            ' class="button" />' %
            (self.__name__, self.__name__, self.label)
            )
...
class FormBase(zc.page.page.Page):
    ...
    def render(self):
        if self.form_reset:
            # We reset, in case data have changed in a way that
...
class EditFormBase(FormBase):
    ...
    @action(_("Edit"), condition=haveInputWidgets)
...
class AddFormBase(FormBase):
    ...
    # zc.page.interfaces.IAddFormCustomization
