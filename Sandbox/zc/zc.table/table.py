...
class Formatter(object):
    ...
    def __init__(self, context, request, items, visible_column_names=None,
        ...
        if interfaces.IColumnSortedItems.providedBy(items):
            items.setFormatter(self) # XXX pull this out to a mixin?
        self.items = items
        if columns is None:
...
def getSortOn(request, sort_on_name, sort_on=None):
    ...
    if sorting:
        ...
        res = {}
        if sort_on:
            for offset, (name, reverse) in enumerate(reversed(sort_on)):
                res[name] = [offset, name, reverse]
...
class SortingFormatterMixin(object):
    ...
    def __init__(self, context, request, items, visible_column_names=None,
        ...
        if not ignore_request:
            sort_on = getSortOn(request, getSortOnName(prefix), sort_on)
        else:
            sort_on = None
...
class AbstractSortFormatterMixin(object):
    ...
    def _addSortUi(self, header, column):
        ...
        template = """
            <span onclick="javascript: %(script_name)s(
...
class StandaloneSortFormatter(
    StandaloneSortFormatterMixin, SortingFormatterMixin, Formatter):
    pass

class FormSortFormatter(
    FormSortFormatterMixin, SortingFormatterMixin, Formatter):
    pass
...
class FormFullFormatter(
    SortingFormatterMixin, FormSortFormatterMixin,
    AlternatingRowFormatterMixin, Formatter):
    pass
