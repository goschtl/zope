...
be no sorting information.
    ...
    <thead>
      <tr>
        <th>
                <span onclick="javascript: onSortClickForm(
                            'First', 'sort_on')"
                        onMouseOver="javascript: this.className='sortable'"
...
The sort_on value can also be used; it is used as initial values, and the 
values in the request modify it.  For instance, if we re-use the big_items
collection from above and pass a sort_on and then modify the request to
effectively get a sort_on of (('First', True), ('Third', False), 
('Second', False)), then the code will look something like this--notice that
we draw arrows indicating the direction of the primary search.
...
column name may appear in the form field, so this is emulated in the example.
    ...
    <thead>
      <tr>
        <th>
                <span onclick="javascript: onSortClickStandalone(
                            'First', 'slot.first.sort_on')"
                        onMouseOver="javascript: this.className='sortable'"
                        onMouseOut="javascript: this.className=''">
