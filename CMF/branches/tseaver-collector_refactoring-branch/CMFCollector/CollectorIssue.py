...
class CollectorIssue(SkinnedFolder, DefaultDublinCoreImpl):
    """
        ...
        Issues are responsible for:
          ...
          - Managing a list of interested parties, including:

            o the submitter (we track name and email, as well, to
              allow for anonymous submission).

            o assigned supporters

            o "kibitzers" (others who have subscribed to the issue)
    """

    meta_type = 'CMF Collector Issue'
    portal_type = 'Collector Issue'
    ...
    def do_action(self, action, comment,
        ...
        self._incrementActionNumber()
        action_number = self.getActionNumber()
        username = str(getSecurityManager().getUser())
    ...
    def _send_update_notice(self, action, actor,
        ...
        candidates = [self._submitter_id, actor] + list(self.assigned_to())
        ...
        for userid in candidates:
            ...
            if (userid == self._submitter_id) and self._submitter_email:
                if self._submitter_email == email:
                    ...
                    email = self._submitter_email
                if self._submitter_name:
                    name = self._submitter_name
            if email:
        ...
        if recipients:
            ...
            subject = ('%s %s/%2d %s "%s"'
                       % (subject, self.id, self.getActionNumber(),
            ...
            message = cin(sender=sender,
                          ...
                          action=action,
                          actor=actor,
                          number=self.getActionNumber(),
                          security_related=self.isSecurityRelated(),
    ...
    def _entry_header(self, type, user, prefix="= ", suffix=""):
        ...
        if self.getActionNumber():
            lead = t + " - Entry #" + str(self.getActionNumber())
        else:
            lead = t
    ...
    def SearchableText(self):
        ...
        if self.isSecurityRelated():
            values.append( 'security_related' )
    ...
    def unindexObject(self):
        """
            Remove the issue from the catalogs which contain it,
            including both the "internal" catalog of the collector and
            the portal_catalog (either of which may not be present).
        """
        for catalog in self._listCatalogs():
