...
from book.messageboard.interfaces import IMessage, IMessageBoard
from book.messageboard.interfaces import IMailSubscriptions
from book.messageboard.interfaces import ISmileyThemeSpecification
...
class MessageDetails:
    ...
    def body(self):
        """Return the body, but mark up smileys."""
        ...
        if obj is None:
            ...
            theme = ISmileyThemeSpecification(obj).theme
  
        for text, url in getSmileysMapping(self.request, theme).items():
            body = body.replace(
                text,
                '<img src="%s" label="%s"/>' %(url, text))
